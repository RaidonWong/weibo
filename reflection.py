...
from agentscope.parsers import ParserBase, MarkdownJsonDictParser
...
from twitter_env import logger,Registry
...
if TYPE_CHECKING:
    from twittermemory import VectorStoreMemory
...
class Reflection(BaseMemoryManipulator):
    ...
    def parseresult(self, result):
        ...
        self.agent.parser=parser
        res = self.agent.parser.parse(result)
        msg = Msg(
            self.agent.name,
            content=self.agent.parser.to_content(res.parsed),
            role="assistant",
            metadata=self.agent.parser.to_metadata(res.parsed),
        )
        return msg
    ...
    def get_questions(self, texts):
        prompt = "\n".join(texts) + "\n" + QUESTION_PROMPT
        result = self.agent.model(prompt)
        result = self.parseresult(result)
        questions = [q for q in result.content.split("\n") if len(q.strip()) > 0]
    ...
    def query_similarity(
        ...
        maximum_score = None
        for text in texts:
            embedding = self.get_embeddings(text, model="text-embedding-3-small")
            score = []
