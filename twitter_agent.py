...
from twitter_utils import TwitterMessage,TwitterPage,InfoBox,agenerate_response
from twitter_env import TwitterEnvironment,Registry,BaseEnvironment
from logging import getLogger
from agentscope.memory import MemoryBase
from twittermemory import PersonalMemory
from reflection import BaseMemoryManipulator
...
class TwitterAgent(DictDialogAgent):
    ...
    def __init__(
        ...
        self.memory_manipulator: BaseMemoryManipulator = Field(
        default_factory=BaseMemoryManipulator)
        self.role_description: str = Field(default="")
        self.max_retry: int = Field(default=3)
        self.output_parser: ParserBase = Field(default_factory=ParserBase)
    ...
    async def reply(self, current_time: dt, env_description: str = "",x: Optional[Union[Msg, Sequence[Msg]]] = None) -> Msg:
        ...
        if env_description!="":
            msg = Msg(content=env_description, sender='News')
