...
def main(scene_id):
    ...
    #twitter_page={'original_tweet':{'content':"Let's support #Mrtoo Movement!",'name':"JordanPatel"}, "like":0, "retweet":0}
    twitter_page={}
    # Initialization
    ...
    for _ in range(1, MAX_TWEET_ROUND + 1):
        ...
        x= Msg(name="JordanPatel",content="Let's support #Mrtoo Movement!" ,role="assistant", echo=True)
        with msghub(agents,announcement=hint) as hub:
            set_parsers(agents, Prompts.action_parser)
            
            asyncio.run(mysequentialpipeline(sequential,twitter_page,like,retweet,x))
